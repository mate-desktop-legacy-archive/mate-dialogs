...
static void
matedialog_notification_icon_update (void)
{
  GdkPixbuf *pixbuf;
  GError *error = NULL;
  ...
  if (!pixbuf) {
    pixbuf = gdk_pixbuf_new_from_file_at_scale (MATEDIALOG_IMAGE_FULLPATH ("matedialog-notification.png"),
...
static gboolean
matedialog_notification_icon_size_changed_cb (GtkStatusIcon *icon,
					  gint size,
					  gpointer user_data)
{
  icon_size = size;
...
static gboolean
matedialog_notification_icon_activate_cb (GtkWidget *widget,
				      MateDialogData *data)
{
  data->exit_code = matedialog_util_return_exit_code (MATEDIALOG_OK);
...
static gboolean
matedialog_notification_handle_stdin (GIOChannel *channel,
				  GIOCondition condition,
				  gpointer user_data)
{
  MateDialogData *zen_data;

  zen_data = (MateDialogData *)user_data;
  ...
  if ((condition & G_IO_IN) != 0) {
	...
	if (error) {
	  g_warning ("matedialog_notification_handle_stdin () : %s",
		     error->message);
	  g_error_free (error);
	  error = NULL;
	}
	...
	continue;
      ...
      if (!g_ascii_strcasecmp (command, "icon")) {
        icon_stock = matedialog_util_stock_from_filename (value);
...
static void
matedialog_notification_listen_on_stdin (MateDialogData *data)
{
  GIOChannel *channel;
  ...
  g_io_add_watch (channel, G_IO_IN | G_IO_HUP,
		  matedialog_notification_handle_stdin, data);
}

void 
matedialog_notification (MateDialogData *data, MateDialogNotificationData *notification_data)
{
  status_icon = gtk_status_icon_new ();
  g_signal_connect (status_icon, "size-changed",
		    G_CALLBACK (matedialog_notification_icon_size_changed_cb), data);

  if (notification_data->notification_text) {
    gtk_status_icon_set_tooltip_text (status_icon, notification_data->notification_text);
  } else {
    gtk_status_icon_set_tooltip_text (status_icon, _("MateDialog notification"));
  }

  icon_file = g_strdup (data->window_icon);
  icon_stock = matedialog_util_stock_from_filename (data->window_icon);
  ...
  if (notification_data->listen) {
    matedialog_notification_listen_on_stdin (data);
  } else {
    /* if we aren't listening for changes, then close on activate (left-click) */
    g_signal_connect (status_icon, "activate",
		      G_CALLBACK (matedialog_notification_icon_activate_cb), data);
  }
